...
final class GithubRequestFactory implements NextRequestFactory
{
    ...
    public function create(Message\ResponseInterface $previousResponse = null): ?Message\RequestInterface
    {
        ...
        return $this->factory
            ->createRequest('GET', "https://api.github.com/repos/{$this->org}/{$this->repository}/pulls?state=all&per_page=100&page=1")
            ->withHeader('Accept', 'application/vnd.github.v3+json')
            ->withHeader('Authorization', 'Bearer '.$this->token)
            ->withHeader('X-GitHub-Api-Version', '2022-11-28')
            ->withHeader('User-Agent', 'flow-gh-api-fetch');
